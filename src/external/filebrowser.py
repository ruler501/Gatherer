...
class FileBrowserIconView(IconView):
    def __init__(self, **kwargs):
        ...
        super(FileBrowserIconView, self).__init__(**kwargs)

    def entry_released(self, *args):
        if self.touch_cooldown > 0:
            return
        else:
            self.touch_cooldown = 3
            super(FileBrowserIconView, self).entry_released(**args)
            Clock.schedule_once(self.reduce_countdown, 0)

    def reduce_countdown(self, *args):
...
class FileBrowserListView(ListView):
    def __init__(self, **kwargs):
        self.touch_cooldown = 0
        super(FileBrowserListView, self).__init__(**kwargs)
    ...
    def entry_released(self, *args):
        if self.touch_cooldown > 0:
            ...
            self.touch_cooldown = 3
            super(FileBrowserListView, self).entry_released(*args)
            Clock.schedule_once(self.reduce_countdown, 0)
        ...
        for place in places:
            if isdir(user_path + place):
                self.add_node(TreeLabel(text=place, path=user_path +
                                        place), libs)
        self._computer_node = self.add_node(TreeLabel(text=self.computer_string,
...
class FileBrowser(BoxLayout):
    ...
    def _post_init(self, *largs):
        ...
        self.ids.link_tree.root_options = {'text': self.location_string,
                                           'no_selection': True}
